...
Note that we're maximzing overall preference here, however, since a globally optimum solution is a time and resource intensive process, we cap the solver solve time to 50 seconds, and first maximize the audience track, then the team track preference, then the weather and then the distance. Note that each optimization builds upon the last and each runs for a maximum of 50 seconds. The individual optimization also helps account for the different magnitude of variables. We've also implemented a decay factor of 0.9. This essentially gives room for subsequent feature optimzations as instead of giving the constraint of having audience track preferences >= BEST_FOUND, we give it the constraint of audience_track_preferences >= BEST_FOUND*DECAY_FACTOR
